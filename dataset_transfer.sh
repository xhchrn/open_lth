...
python3 open_lth.py lottery_branch change_depth --num_workers 4 --default_hparams=cifar_resnet_20 \
...
python3 open_lth.py lottery_branch change_depth --num_workers 4 --default_hparams=cifar_resnet_20 \
    --dataset_name        $source_dataset --model_name        $source_model --rewinding_steps 1000it --replicate=$replicate \
    --target_dataset_name $target_dataset --target_model_name $target_model --block_mapping "${mapping}" --levels=1
